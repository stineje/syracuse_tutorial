...
The \texttt{gf180mcuD} process offers a 5-metal-layer CMOS flow (plus
local interconnect) suitable for both digital and mixed-signal designs.
...
out of Box mode---and because the cursor change is subtle, it is easy
not to notice.
...
\texttt{li}      & Metal    & Local interconnect (TiN); use sparingly \\
...
Polysilicon has high resistance and capacitance.
Local interconnect (\texttt{li}) is roughly 100$\times$ more resistive
than \texttt{metal1} and should be reserved for very short connections
...
\midrule
Local interconnect & \texttt{li}     & Short hops; use sparingly \\
...
\begin{tipbox}
Try to label supply wires as \texttt{vdd!} and \texttt{gnd!}---not
\texttt{VDD}, \texttt{GND}, or any other variant.  This is optional!
Magic's extraction engine uses these exact strings to identify global
supply nets during LVS.
This is one of the most common sources of LVS failures for new users.
The exclamation mark is not decorative---it is the character that tells
Magic the net is \emph{global}, meaning it will automatically connect to
every other net carrying the same label anywhere in the hierarchy,
without requiring an explicit port or wire connection between cells.
Omitting it, or using uppercase letters, produces a net that Magic
treats as an ordinary local wire, which will either float or mismatch
against the schematic during LVS.
While it is technically possible to complete a small layout without
placing any supply labels---Magic will not stop you---at least one
\texttt{vdd!} and one \texttt{gnd!} label must appear somewhere in the
design for extraction and LVS to function correctly.
A layout with no supply labels will extract cleanly but produce an LVS
result in which every transistor appears to have floating source and
drain connections, making the comparison meaningless.
In practice:
\begin{itemize}
\item \textbf{Use exactly} \texttt{vdd!} and \texttt{gnd!} --- all lowercase, with the exclamation mark, no spaces.
\item \textbf{Place at least one of each} somewhere in the top-level layout, even if the label is on a wire that runs off-screen or connects only to a power ring.
\item \textbf{Do not rely on implicit naming} --- if a wire is visually connected to the supply but carries no label, extraction will not recognize it as a supply net.
\end{itemize}
\end{tipbox}
...
When in doubt, start at minimum and increase only when there is a
...
again.''
The consequences in industry are severe: I have heard from colleagues that
several companies explicitly
list bypassing LVS as grounds for immediate dismissal.
...
Commercial tools such as Mentor/Siemens \textbf{Calibre~DRV} are the
